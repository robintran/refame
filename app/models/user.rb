class User < ActiveRecord::Base
  ...
  has_many :account_credentials, :dependent => :destroy
  ...
  def self.autogenerated_password
    Devise.friendly_token.first(6)
  end

  def self.autogenerated_twitter_email(username)
    "#{username}@twitter.com"
  end

  def self.find_user(provider, uid)
    AccountCredential.find_by_account_type_and_uid(provider, uid).try(:user)
  end
end
